...
static ssize_t enable_read(struct file *file, char __user *buf, size_t lbuf, loff_t *ppos)
{
	...
	return simple_read_from_buffer(buf, lbuf, ppos, kstring, nbytes);//内核复制数据到用户空间
}
...
static ssize_t enable_write(struct file *file, const char __user *buf, size_t lbuf, loff_t *ppos)
{
	...
	oldparm = enable_parm;
	rc = simple_write_to_buffer(kstring, lbuf, ppos, buf, lbuf);//buf->kstring,用户到内核
...
static int lock_info_open(struct inode *inode, struct file *file)
{
	...
	printk("this is lock_info_read");
	int ret;
	ret=single_open_size(file, lock_info_print, NULL, size);
	if(ret!=0)
		printk("single_open_size error");
...
{
if(filter_key==0&&filter_address==0)
	seq_puts(p,filter_str);
else
{
char filterbuf[32];
sprintf(filterbuf,"%ld %lx\n",filter_key,filter_address);
seq_puts(p,filterbuf);
...
static int stack_output_print(struct seq_file *p, void *v)
{
    ...
    printk("this is stack_output_print");
	struct task_info *task_info;
	struct lock_info *lock_info;
	...
	{
	    printk("find task_info in stack_output_print");
		...
		list_for_each_entry(lock_info, &(task_info->locks.head), node)
		{
			if (lock_info->lock_addr == filter_address)
			{
				...
				{
					seq_printf(p, "[%02d]-[%p] %pS\n", i, (void *)lock_info->stack_entries[i], (void *)lock_info->stack_entries[i]);//%pS将地址转换为函数名+偏移量
				}
...
{
	unsigned int size = PAGE_SIZE;
    printk("this is stack_output_read");
	return single_open_size(file, stack_output_print, NULL, size);
}
static int stack_output_release(struct inode *inode, struct file *file)
{
    printk("this is stack_output_release");
    return single_release(inode, file); // 释放seq_file
}
static int lock_info_release(struct inode *inode, struct file *file)
{
    printk("this is lock_info_release");
    return single_release(inode, file); // 释放seq_file
}
static int filter_release(struct inode *inode, struct file *file)
{
    printk("this is filter_release");
    return single_release(inode, file); // 释放seq_file
}
...
// all ops-func
struct file_operations fops[node_num] = {
    {
        .read = enable_read,    // 对应原proc_read
        .write = enable_write   // 对应原proc_write
    },
    {
        .read = threshold_read,  // 对应原proc_read
        .write = threshold_write // 对应原proc_write
    },
    {
        .read = savetime_read,   // 对应原proc_read
        .write = savetime_write  // 对应原proc_write
    },
    {
        .open = lock_info_open,  // 对应原proc_open
        .read=seq_read,
        .llseek=seq_lseek,
        .release=lock_info_release
        
    },
    {
        .open = filter_open,     // 对应原proc_open
        .write = filter_write,    // 对应原proc_write
        .read=seq_read,
        .llseek=seq_lseek,
        .release=filter_release
    },
    {
        .open = stack_output_open, // 对应原proc_open
        .read=seq_read,
        .llseek=seq_lseek,
        .release=stack_output_release
    }
};
